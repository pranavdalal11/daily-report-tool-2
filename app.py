...
def _init_db_startup() -> None:
    strict = (os.environ.get("DB_INIT_STRICT") or "").strip().lower() in {"1", "true", "yes"}
    attempts = 25 if strict else 2
    sleep_seconds = 1.0 if strict else 0.2
    last_error = None
...
if (os.environ.get("DB_INIT_STARTUP") or "").strip().lower() in {"1", "true", "yes"}:
    try:
        _init_db_startup()
    except Exception as e:
        import sys
